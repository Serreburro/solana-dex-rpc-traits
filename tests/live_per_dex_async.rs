...
async fn orca_bidirectional_live() {
    let pools = client()
        .get_all_orca_amm_pools_by_tokens_bidirectional(&pubkey(USDC), &pubkey(WSOL), true)
        .await
        .unwrap();
...
async fn orca_single_token_live() {
    let pools = client()
        .get_all_orca_amm_pools_by_token(&pubkey(USDC), true)
        .await
        .unwrap();
...
async fn meteora_dlmm_bidirectional_live() {
    let pools = client()
        .get_all_meteora_dlmm_amm_pools_by_tokens_bidirectional(&pubkey(USDC), &pubkey(WSOL), true)
        .await
        .unwrap();
...
async fn meteora_dlmm_single_token_live() {
    let pools = client()
        .get_all_meteora_dlmm_amm_pools_by_token(&pubkey(USDC), true)
        .await
        .unwrap();
...
async fn meteora_damm_v2_bidirectional_live() {
    let pools = client()
        .get_all_meteora_damm_v2_amm_pools_by_tokens_bidirectional(&pubkey(USDC), &pubkey(WSOL), true)
        .await
        .unwrap();
...
async fn meteora_damm_v2_single_token_live() {
    let pools = client()
        .get_all_meteora_damm_v2_amm_pools_by_token(&pubkey(USDC), true)
        .await
        .unwrap();
...
async fn meteora_damm_v1_bidirectional_live() {
    let pools = client()
        .get_all_meteora_damm_v1_amm_pools_by_tokens_bidirectional(&pubkey(USDC), &pubkey(WSOL), true)
        .await
        .unwrap();
...
async fn meteora_damm_v1_single_token_live() {
    let pools = client()
        .get_all_meteora_damm_v1_amm_pools_by_token(&pubkey(USDC), true)
        .await
        .unwrap();
...
async fn raydium_clmm_single_token_live() {
    let pools = client()
        .get_all_raydium_clmm_amm_pools_by_token(&pubkey(USDC), true)
        .await
        .unwrap();
...
async fn pancake_swap_bidirectional_live() {
    let pools = client()
        .get_all_pancake_swap_amm_pools_by_tokens_bidirectional(&pubkey(USDC), &pubkey(WSOL), true)
        .await
        .unwrap();
...
async fn pancake_swap_single_token_live() {
    let pools = client()
        .get_all_pancake_swap_amm_pools_by_token(&pubkey(USDC), true)
        .await
        .unwrap();
...
async fn byreal_bidirectional_live() {
    let pools = client()
        .get_all_byreal_amm_pools_by_tokens_bidirectional(&pubkey(USDC), &pubkey(WSOL), true)
        .await
        .unwrap();
...
async fn byreal_single_token_live() {
    let pools = client()
        .get_all_byreal_amm_pools_by_token(&pubkey(USDC), true)
        .await
        .unwrap();
...
async fn pumpfun_bidirectional_live() {
    let pools = client()
        .get_all_pump_fun_amm_pools_by_tokens_bidirectional(&pubkey(random), &pubkey(WSOL), true)
        .await
        .unwrap();
...
async fn pumpfun_single_token_live() {
    let pools = client()
        .get_all_pump_fun_amm_pools_by_token(&pubkey(USDC), true)
        .await
        .unwrap();
...
async fn raydium_cpmm_bidirectional_live() {
    let pools = client()
        .get_all_raydium_cpmm_amm_pools_by_tokens_bidirectional(&pubkey(USDC), &pubkey(WSOL), true)
        .await
        .unwrap();
...
async fn raydium_cpmm_single_token_live() {
    let pools = client()
        .get_all_raydium_cpmm_amm_pools_by_token(&pubkey(USDC), true)
        .await
        .unwrap();
...
async fn raydium_amm_bidirectional_live() {
    let pools = client()
        .get_all_raydium_amm_pools_by_tokens_bidirectional(&pubkey(USDC), &pubkey(WSOL), true)
        .await
        .unwrap();
...
async fn raydium_amm_single_token_live() {
    let pools = client()
        .get_all_raydium_amm_pools_by_token(&pubkey(USDC), true)
        .await
        .unwrap();
...
async fn saros_bidirectional_live() {
    let pools = client()
        .get_all_saros_amm_pools_by_tokens_bidirectional(&pubkey(USDC), &pubkey(WSOL), true)
        .await
        .unwrap();
...
async fn saros_single_token_live() {
    let pools = client()
        .get_all_saros_amm_pools_by_token(&pubkey(USDC), true)
        .await
        .unwrap();
...
async fn orca_v2_bidirectional_live() {
    let pools = client()
        .get_all_orca_v2_amm_pools_by_tokens_bidirectional(&pubkey(USDC), &pubkey(WSOL), true)
        .await
        .unwrap();
...
async fn orca_v2_single_token_live() {
    let pools = client()
        .get_all_orca_v2_amm_pools_by_token(&pubkey(USDC), true)
        .await
        .unwrap();
...
async fn orca_v1_bidirectional_live() {
    let pools = client()
        .get_all_orca_v1_amm_pools_by_tokens_bidirectional(&pubkey(USDC), &pubkey(WSOL), true)
        .await
        .unwrap();
...
async fn orca_v1_single_token_live() {
    let pools = client()
        .get_all_orca_v1_amm_pools_by_token(&pubkey(USDC), true)
        .await
        .unwrap();
...
async fn stepn_bidirectional_live() {
    let pools = client()
        .get_all_stepn_amm_pools_by_tokens_bidirectional(&pubkey(USDC), &pubkey(WSOL), true)
        .await
        .unwrap();
...
async fn stepn_single_token_live() {
    let pools = client()
        .get_all_stepn_amm_pools_by_token(&pubkey(USDC), true)
        .await
        .unwrap();
...
async fn token_swap_bidirectional_live() {
    let pools = client()
        .get_all_token_swap_amm_pools_by_tokens_bidirectional(&pubkey(USDC), &pubkey(WSOL), true)
        .await
        .unwrap();
...
async fn token_swap_single_token_live() {
    let pools = client()
        .get_all_token_swap_amm_pools_by_token(&pubkey(USDC), true)
        .await
        .unwrap();
...
async fn penguin_bidirectional_live() {
    let pools = client()
        .get_all_penguin_amm_pools_by_tokens_bidirectional(&pubkey(USDC), &pubkey(WSOL), true)
        .await
        .unwrap();
...
async fn penguin_single_token_live() {
    let pools = client()
        .get_all_penguin_amm_pools_by_token(&pubkey(USDC), true)
        .await
        .unwrap();
...
async fn fusion_bidirectional_live() {
    let pools = client()
        .get_all_fusion_amm_pools_by_tokens_bidirectional(&pubkey(USDC), &pubkey(WSOL), true)
        .await
        .unwrap();
...
async fn fusion_single_token_live() {
    let pools = client()
        .get_all_fusion_amm_pools_by_token(&pubkey(USDC), true)
        .await
        .unwrap();
...
async fn obric_v2_bidirectional_live() {
    let pools = client()
        .get_all_obric_v2_amm_pools_by_tokens_bidirectional(&pubkey(USDC), &pubkey(WSOL), true)
        .await
        .unwrap();
...
async fn obric_v2_single_token_live() {
    let pools = client()
        .get_all_obric_v2_amm_pools_by_token(&pubkey(USDC), true)
        .await
        .unwrap();
...
async fn futarchy_bidirectional_live() {
    ...
    let pools = client()
        .get_all_futarchy_amm_pools_by_tokens(&pubkey(USDC), &pubkey(solo_token), true)
        .await
        .unwrap();
...
async fn futarchy_single_token_live() {
    let pools = client()
        .get_all_futarchy_amm_pools_by_token(&pubkey(USDC), true)
        .await
        .unwrap();
