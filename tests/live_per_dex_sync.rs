...
fn pumpfun_bidirectional_live() {
    let pools = client()
        .get_all_pump_fun_amm_pools_by_tokens_bidirectional(&pubkey(USDC), &pubkey(WSOL))
        .unwrap();
...
fn futarchy_bidirectional_live() {
    ...
    let pools = client()
        .get_all_futarchy_amm_pools_by_tokens_bidirectional(&pubkey(USDC), &pubkey(solo_token))
        .unwrap();
